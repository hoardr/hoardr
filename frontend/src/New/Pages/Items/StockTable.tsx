...
export function StockTable({stockItems, unit, columns: selectedColumns = ['location']}: { stockItems: StockItem[], unit?: Unit, columns?: ("location" | "item")[] }) {
    ...
    const columns: Columns<StockItem> = [
        {
            ...
            key: "item",
            title: <IconText icon={ViewGridIcon}>Item</IconText>,
        ...
        },
        {
            key: "quantity",
            title: "Quantity",
            render: (stockItem) => <span className={"text-gray-500"}>{quantity(stockItem.quantity, unit ?? stockItem.item.unit)}</span>
        },
        {
            key: "edit",
    ...
    return <Table keyIndex={(stockItem) => stockItem.location?.id ?? stockItem.item.id} columns={filtered} data={stockItems}/>
}
